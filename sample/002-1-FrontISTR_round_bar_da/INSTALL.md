...
`fistr1` が使えるようになったら、丸棒熱伝導ケースの `.msh` と `.cnt` を使い、OpenFOAM 版と同じ OI ループに接続します。

`case/round_bar.cnt` はテンプレートとして置いてあり、実行時には `fistr_interface.py` が毎ステップで上書きします。
具体的には、初期温度、右端の固定温度、左端の熱流束をその時点の同化結果に合わせて書き換えます。
