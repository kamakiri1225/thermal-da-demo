...
動きで見るとこうです（加熱でだんだん反り、冷却で戻る）:

![熱膨張による変形のアニメーション](img/da_deform_compare.gif)

*時間を追った熱変形アニメーション。片側加熱で円筒がゆっくり反っていくのが分かる。
（変形は見やすさのため誇張表示。これが「正解値（真値）」の変形で、あとの同化で復元を目指す対象になる。）*
...
![熱伝達率を色分けした3D図（どの面がどのh か）](img/blog_htc_paraview.png)
