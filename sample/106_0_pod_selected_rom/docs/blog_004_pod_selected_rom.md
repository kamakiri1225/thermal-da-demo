...
**目的は、第3回のEnKFで毎回必要になる予報を軽くし、少数の温度状態から全温度場・変位まで復元することです。**
この記事では、既存のOpenFOAM温度履歴を入力にして、次の順にモデルを作ります。

| 手順 | 入力 → 出力 | 担当する実装 |
|---|---|---|
| POD | 各セルの温度履歴 → 各セルの平均場・空間モード | `run/select_points_qdeim.py` |
| Q-DEIM | 空間モード → 5代表セル | 同上 |
| 熱ROMの校正 | 5セルの時刻歴 → 熱容量・結合・放熱係数 | `run/build_calibrate_rom.py` |
| 予報・同化 | 各員の5温度・発熱倍率・放熱係数 → 更新状態 | `dacore/rom_general.py`、`dacore/enkf.py` |
| 全場復元 | 更新後5温度 → モード係数 → 全セル温度 | gappy-POD（§5） |
| 変位復元 | モード係数 → 変位 | 事前計算したFrontISTRモード応答（§7） |

**5代表点は実際のセンサ5個を意味しません。** 代表点はROMの状態、観測点はその状態を補正するための測定位置です。
同化する観測の数と位置は別に選びます。また、このROMはPODで熱方程式を直接射影したものではなく、POD/Q-DEIMで選んだ点に**集中定数モデルを校正**したものです。

理論を追う順は **§2のPOD → §3の手計算 → §4の点選択 → §5の全場復元 → §6の時間発展 → §7の変位**。
§2-5のクリロフ法は補足です。主筋を先に読む場合は§3へ進み、後から戻れます。式展開は省略せず残しています。
...
flowchart LR
    ...
    D --> E["④ 軽いROMでEnKF/OI<br/>単一ROMの前進積分が約31ms"]
```
...
この単一ROMの計測値は、全場復元・変位評価・図生成を含むEnKF一式の計算時間ではありません。**予報モデルをROMに差し替えるだけ**で、
...
**§2〜5の $u$ は温度場です。§7の $\boldsymbol u$ は変位です。** 既存の式を追う際は、温度の $u$ をノートでは $T$ と書き換えて構いません。温度と変位を同じ物理量として扱わないでください。

| 記号 | 大きさ・単位 | 意味 |
|---|---|---|
| $N,m,r$ | 20,696、121、5 | セル数、保存時刻数、採用モード数 |
| $S,X$ | $N\times m$ 、K | 温度履歴、各セルの時間平均を引いた履歴 |
| $\bar u$ | $N\times1$ 、K | 各セル固有の時間平均温度 |
| $U,\Sigma,V^{\mathsf T}$ | 薄いSVDで $N\times m,m\times m,m\times m$ | 左特異ベクトル、特異値、右特異ベクトルの転置 |
| $\Phi=U_r$ | $N\times r$ 、無次元 | 上位r本の空間モード |
| $a(t)$ | $r\times1$ 、K | 時刻tで全セルが共有するモード係数 |
| $P$ | $5\times N$ 、無次元 | 選んだセルの行を取り出す選択行列 |

$\mathsf T$ は転置、 $^+$ はMoore–Penrose疑似逆行列、 $\sigma_k$ は特異値、 $\lambda_k$ は固有値です。
本文の「r点P」は選んだ点集合を指します。行列として書くと $U_{r,P}=P\Phi$ です。
PODの「エネルギー」は温度変動の二乗和であり、熱量[J]ではありません。この実装はセル体積の重みを付けないSVDです。

平均場と基底は0〜600秒の121枚から作って固定します。300秒でヒータを切っても平均場だけを切り替えません。
...
制約 $\varphi^{\mathsf T}\varphi=1$ をラグランジュ乗数 $\lambda$ で組み込みます。

$$
\mathcal L=\varphi^{\mathsf T}XX^{\mathsf T}\varphi-\lambda(\varphi^{\mathsf T}\varphi-1).
$$

対称行列Aに対して $\partial(\varphi^{\mathsf T}A\varphi)/\partial\varphi=2A\varphi$ なので、

$$
\frac{\partial\mathcal L}{\partial\varphi}=2XX^{\mathsf T}\varphi-2\lambda\varphi=0.
$$

両辺を2で割って移項すると、次の固有値問題になります:
...
$r$ 個の未知を決めるには **$r$ 個の独立な測定**があり、選点行列がフルランクなら係数を決められる ―― これが「少数点でROMを組める」理由です。
...
#### 「高感度／低感度」はどういうロジックで決めているか

本研究では **温度センサと変位センサで、感度の定義と決め方が違います**。混同しやすいので、レシピ形式でまとめます。

**① 温度センサの高/低感度 ＝ 発熱への温度感度 $\partial T/\partial Q$**

> 「ヒータの発熱が変わったとき、**温度がよく反応する場所**はどこか」

1. 発熱を10%増やして（ $q_\mathrm{scale}$ : 1.0→1.1）ROMをもう一度回す
2. 加熱ピーク（ $t=300$ s）で、各点の温度上昇の差をとる： $s_i=\dfrac{T_i(1.1Q)-T_i(Q)}{0.1Q}$
3. 大きい順に並べる → **最大＝高感度（P2＝ヒータ最近傍、5.7）**、**最小＝低感度（P4＝反対側の底面、2.6）**

物理的な理屈：**熱源に近いほど発熱変化が薄まらずに届く**ので反応が大きい。遠い点は拡散でならされ、反応が鈍い。

**② 変位センサの高/低感度（高W/低W）＝ 温度への変位感度 $W=K_s^{-1}H_T$**

> 「温度が変わったとき、**変位がよく動く場所**はどこか」

1. FrontISTRの熱弾性（ $K_s u=H_T T$ ）を1度解いて、温度→変位の行列 $W$ を作る
2. 候補点ごとに **$W$ の行ノルム**（＝その点の変位が温度場にどれだけ反応するか）を計算
3. 大きい順に並べる → **高W＝上端の点**、**低W＝底面の点**

物理的な理屈：**底面は固定**なので、そこから遠い上端ほど各層の伸びが**累積**して現れる（てこの先端ほど動く）。
底面近くは拘束されていてほとんど動けない＝測っても情報が少ない。

**まとめ表**：

| | 温度センサ | 変位センサ |
|---|---|---|
| 感度の定義 | $\partial T/\partial Q$ （発熱→温度） | $W=K_s^{-1}H_T$ の行ノルム（温度→変位） |
| 決め方 | $Q$ を10%振って温度差を見る | FrontISTRを1回解いて $W$ の行を比べる |
| 高感度の場所 | **ヒータ最近傍**（P2） | **上端**（固定面から最遠） |
| 低感度の場所 | 反対側の底面（P4） | 底面（固定で動けない） |
| どちらも意味は | 「測る量が、知りたい変化に**よく反応する**場所」を選ぶ、という同じ発想 | |

（詳しい導出は blog_002 §4 と `105_0_sensor_placement_sensitivity/docs/03_sensor_design_guide.md` にあります。）

<img src="img/sensor_positions.png" alt="5温度点と変位観測点A/Oの位置" width="520">
...
$T_r$ は同化後の5代表点温度（全5点の直接測定値ではありません）。全場を $T_{\rm full}\approx\bar u+\Phi a$ と表すと、

$$
T_r=P T_{\rm full}\approx P\bar u+P\Phi a.
$$

平均を引いて $(P\Phi)a=T_r-P\bar u$ を解き、全場の式へ戻すと、

$$
a=(P\Phi)^+(T_r-P\bar u),\qquad
\widehat T_{\rm full}=\bar u+\Phi(P\Phi)^+(T_r-P\bar u).
$$

$P\Phi$ は5×5。フルランクなら逆行列と疑似逆行列は一致します。一般には、選んだ点での残差二乗和を最小にする係数を疑似逆行列で求めます。観測点数不足やモード外の温度変化があれば、全場を一意・正確に復元できるとは限りません。

「gappy」は全場の一部にしか値がない状態を指します。この処理は距離補間ではなく、学習した空間モードによる復元です。

#### この式の読み方 ―「モードの割合」で各点の温度が決まる

**各時刻のモード係数 $a_k$ が、固定したモードをどれだけ足すかを決めます**。係数は負にもなり、合計1の割合ではありません。式の2つの部品は役割がはっきり違います:
...
**P0–P2 が最強（ $K=17.5$ W/K）**＝ヒータ側どうしが強く熱をやり取りする。
...
**ここからは空間の移し替えです。** 5点→全セルの復元は§5のgappy-PODで済んでいます。その後、CFDセルとFEM節点の位置の違いを補間します。以下はIDWの説明で、すべての図が同じ補間実装という意味ではありません。例えば未観測C/Dの検証スクリプトは最近傍セルを使います。各図の再現スクリプトで確認してください。
...
$\varepsilon_{\rm th}$ は熱ひずみ、 $\mathbf I$ は3×3単位テンソル、 $\Omega_e$ は要素eの領域、 $dV$ は体積積分の微小体積です。
$B$ は節点変位からひずみを作る行列、 $D$ はひずみから応力を作る弾性行列、 $f_{\rm th}$ は等価熱荷重[N]、 $K_s$ は構造剛性[N/m]です。
**このBはOIの誤差共分散ではなく、このDはPODモード変位応答でもありません。**
テンソルの熱ひずみを実装のVoigt表記に直すと、 $\alpha\Delta T(1,1,1,0,0,0)^{\mathsf T}$ です。後半3成分はせん断ひずみです。
...
**表示の条件**：`run/make_temp_to_disp_figs.py`は、保存した1つのFrontISTR変位形状をA−O時系列の振幅で拡大縮小して動画にしています。色も元の変位場を使用します。各フレームでFrontISTRを解き直した変形時刻歴ではなく、反りの変化を説明する模式アニメーションです。定量的な時刻歴は次節のグラフで確認します。
...
同化の精度を確認するときは、同じ量に **ROM真値温度をFrontISTRの変位写像へ通した参照値を黒太線**で重ねます。次の図は
真値（黒）と、温度だけ／温度＋変位を観測したデータ同化後の推定値（色線）を同じ
$U_z(A)-U_z(O)$ で比較したものです。薄い帯は5 seedのばらつきです。図内に「FrontISTR真値」とある場合も、この比較の真値はROM温度からFrontISTRのモード応答を用いて計算したものを指します。

![ROM真値とデータ同化後温度から求めた変位差](img/blog_disp_timeseries_truth_vs_da.png)

*黒太線はROM真値から同じ変位写像で計算した参照値。独立したOpenFOAM温度履歴を毎時刻FrontISTRで解析した真値ではありません。赤（温度2点＋変位2点）は真値に最も近く、
温度観測だけ（緑）は変位差に誤差が残る。これは変位を状態変数として直接積分した結果ではなく、
同化後温度をgappy-PODで全場へ戻し、その温度をFrontISTRの熱弾性写像へ通して得た推定値です。*

**この図の正直な読み**：変位差RMSEで比べると、温度センサの**場所（低感度1.19 vs 高感度0.99 µm）や数（2点で0.93 µm）の差はわずか**です。桁で効くのは**変位観測の追加**（0.17 µm＝約6倍改善）。
これは「Q/hへの温度感度が空間的にほぼ一様」という直接感度の結果とも整合します。
これは今回の配置・ノイズ・モデルでの結果です。**一般に温度センサの場所より数が重要だと結論することはできません。**
（なお温度そのもののRMSEでは高感度0.62 vs 低感度1.04 Kと約1.7倍の差があり、場所の効きは評価量に依存します。）

**さらに大事な注意 ―「差」だけで評価してはいけない**。個別の $U_z(A)$ ・ $U_z(O)$ も見ると:

![個別のUz(A)/Uz(O)と差の比較](img/blog_disp_timeseries_truth_vs_da_points.png)

*「差」には罠があります。**同化なし（灰）は個別には10µm級で大外れなのに、差のRMSEは0.30µmと最小級**
（A・Oが同じ方向に外れて相殺されるため）。個別で見ると、**温度1点は「センサに近い側」しか合いません**
（P2ヒータ側→A良0.41／O悪1.33µm、P4底→O良0.65／A悪1.75µm）。**変位2点（赤）だけが両点とも0.16µm**で合う。
→ 教訓：**変位を合わせたい場所には変位観測を置く。そして評価は差だけでなく個別の変位でも行う**。*

**仕上げに、まったく別の未観測点でも確認**します。観測点（赤/橙）から離れた
**未観測の温度3点（緑）と変位3点（紫）**の位置はこちら:

<img src="img/unobs_points_locations.png" alt="観測点と未観測評価点の位置" width="440">

この6か所すべてで、真値と各構成を時系列比較すると:

![未観測の温度3点・変位3点の時系列](img/unobs_multi_timeseries.png)

*同化なし（灰）は6点すべてで大外れ。同化した構成は**観測していない場所でも**真値に沿い、
温度＋変位（赤）が最も近い（特に底面側の温度と z=75mm 反対側の変位の立ち上がり）。
**少数の観測が、観測していない場所まで直す**――これがデータ同化＋POD復元の価値です。*

> **凡例の「高感度／低感度」の意味**：ここでの感度は **発熱量への温度感度 $\partial T/\partial Q$** です
> （ $q_\mathrm{scale}$ を1.0→1.1に振った有限差分、加熱ピーク $t=300$ s で評価）。
> 高感度＝ $\partial T/\partial Q$ 最大のP2（ヒータ側）、低感度＝最小のP4（反対側）、
> 温度2点(高感度)＝上位2ノード（P2とP0）。上の観測点図（sensor_positions）の色付けと同じ量です。
> なお **変位センサ選びに使う熱感度 $W=K_s^{-1}H_T$ （温度→変位）とは別物**なので注意
> （温度センサは $\partial T/\partial Q$ 、変位センサは $W$ で選ぶ、という役割分担）。

> 実務では、この温度→変位を毎回FrontISTRで解く代わりに、**線形化した感度 $W=K_s^{-1}H_T$**（blog_002 §4）で
> 温度から直接変位を出せます。 $W$ は多数の温度入力に対する線形応答です。剛性行列の分解を再利用できますが、単一の熱荷重を1回解くだけで全Wが得られるわけではありません。POD版では必要なモード応答だけを事前に計算します。
...
1. **EnKFは実ソルバだと重い** → 予報モデルを軽い ROM に置き換える（単一ROMの前進積分が約31ms）。
