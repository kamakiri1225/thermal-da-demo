...
この配置で、温度P2 に **①変位なし ②低W変位2点 ③高W変位2点** を足した3構成を、同じ真値・初期条件で
**OI（106の5点温度ROM、背景共分散 $B$ を固定）** で回し、全5点温度RMSEを比べました（5seed平均）。

![OIで高W／低W変位点を比べた温度RMSE](img/blog_oi_disp_selection.png)

*これは **OI（最適内挿）によるデータ同化の結果**（固定 $B$ ・ROM）。**高W（青）は速く下がる**が、
**低W（橙）は変位なし（灰）とほとんど変わらない**。*

| 観測構成（OI） | 加熱期(0-300s)の平均温度RMSE |
|---|---|
| 温度1点のみ（変位なし） | 3.52 K |
| ＋変位2点（低W） | 2.43 K（あまり効かない） |
| **＋変位2点（高W）** | **0.86 K（約4倍改善）** |
...
![σQ/σTスタディの観測点・評価点の位置](img/oi_evaluation_locations.png)

*左＝側面投影(x-z)、右＝上面(x-y)。**同化の観測に使う**のは、温度 **T1=hot（ヒータ側+X・中央高さ）／T2=cold（反対側−X・中央高さ）** の2点と、
上面変位 **A/O**（+X・−X の節点列の平均）。**観測に使わない"評価だけ"の点**は、温度 **T3/T4**（上部・下部の未観測点）と、
変位 **C/D**（中高さ z=75 mm）。C−D差は「同化に使っていない別の変位差」で、**同化がその点まで当たるか（汎化）**を見る評価量です。
（右図：A/Oは単点でなく上面の +X/−X 節点列の平均。）*
