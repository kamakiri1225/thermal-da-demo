...
> （ΔQoI = Σ W_diff[j]·ΔT[j]）で決まる。「+X側下部が濃い」のは、QoIの定義点A
> をたまたまヒータ側に選んだから＋下を温めると上の柱全体が持ち上がるからで、
> ヒータを反対側に移してもこの地図は変わらない。
> （A側を温めるとQoI+なので+X側が赤、O側を温めるとQoI−なので−X側が青）
...
2. **固定境界近傍の見かけ感度は捨てる**。拘束節点まわりはアーティファクトで
   感度が桁違いに出る（今回も6×10⁷µm/Kが出現。中央値の100倍超は除外）
