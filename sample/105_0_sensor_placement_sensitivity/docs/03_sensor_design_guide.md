...
このデータ同化で推定したいのは**円筒全体の温度分布**（5040節点。直接は数点しか
測れない）と発熱量Q。変位計はそれを間接的に知るための道具で、点iの読み値は

$$u_i = W[i,1]\Delta T_1 + W[i,2]\Delta T_2 + \cdots + W[i,5040]\Delta T_{5040}$$

という「**未知の温度5040個の重み付き合計**」＝温度場についての方程式1本になる。

![変位計1本=方程式1本](img/guide_equation.png)
- 係数W[i,:]が大きい点：温度場が変わると読み値がはっきり動く
  → 読み値から温度場を強く絞り込める（**使える方程式**）
- 係数がほぼゼロの点：温度場がどう変わっても読み値が動かない
  → 方程式が「0≈0＋ノイズ」になり温度場について何も教えない

「情報が多い」とは、この**方程式の係数が大きい**ことを指す。

**左の地図は具体的に何から計算したものか？**

出発点は W[i,j] =「**節点jだけを+1℃温めたとき、点iの上下変位uzが何µm動くか**」
（実際の固定条件込みのFEM応答。K⁻¹Hで全組み合わせを一括計算）。

左の地図の点iの色は、これを**温める場所jについて全節点分（5040通り）足した**もの：

$$\text{left color}(i) = \sum_{j=1}^{5040} \big| W[i,j] \big| \quad [\mu m/K]$$

言い換えると「**円筒のどこか1か所を1℃温める実験を5040通り全部やって、
点iの変位計が動いた量（絶対値）を合計した**」値。本来なら5040回のFEM実験が
要るところを、K⁻¹Hの1回の計算で全部まとめて得ている。

- この合計が**大きい点**（上端、6.33µm/K）＝どこが温まっても変位計がよく反応する
  → 読み値に温度場の情報が濃く乗る → 貼る価値が高い
- **小さい点**（底面近く、0.12µm/K）＝円筒のどこが温まってもほぼ動かない
  → 読み値はほぼノイズだけ → 貼っても情報が取れない
...
**左右どちらも「熱感度」— 違いは何を固定して何を動かすか**

| | 左の地図（行ノルム） | 右の地図（W_diff） |
|---|---|---|
| 色が答える質問 | 「**ここで変位を測ったら**、温度場の変化にどれだけ反応するか」 | 「**ここの温度が1℃動いたら**、決まった観測（QoI）がどれだけ動くか」 |
| 場所xの色の意味 | xは**変位計を貼る候補地** | xは**温度が変わりうる場所** |
| 使い道 | 変位計をどこに貼るか | どこの温度を管理すべきか |
| Wでの計算 | 行ごとの合計 Σⱼ\|W[i,j]\| | 特定の行の差 W[A,:]−W[O,:] をそのまま表示 |

同じ場所でも色の意味は逆向きになる。例えば**円筒の上端**は、
左では真っ赤（上端はよく動く→変位計を貼るなら上端）だが、
右では薄い（上端を温めても持ち上げる相手がない→上端の温度はQoIをほぼ動かさない）。
