...
> **先に大事な整理：この5点ROMは POD を使わずに作った**（物理直観の集中定数モデルを校正）。
> **ROM化にPODは必須ではない。** ここでPODを持ち出すのは、できたROMを
> **診断する（何自由度要るか／分布は失われないか／代表点は妥当か）ため**であって、
> ROMを作るためではない。
> - POD不要でよい人：代表点が決まっていて精度も足りている → この章は「なぜ5点で足りるか」の裏付けとして読めばよい
> - PODが要る人：新しい系で必要自由度が分からない／分布保存を証明したい／点を最適化したい／
>   物理モデルを立てずデータだけでROMを作りたい（POD-Galerkin）
> 使うタイミングは**設計時に1回だけ**で、装置を運転しているあいだ（STEP2）には使わない。
...
> $T(x)=\left(\sum_i w_i T_i\right)/\left(\sum_i w_i\right)$ 、
> 重み $w_i = 1/\lVert x-p_i\rVert^2$ （距離の逆二乗）で決める。近い点ほど大きく効く。
...
\quad(20{,}696\text{ rows}\times121\text{ cols})
$$
...
> **記号の読み方（大事）**： $u(x,t)$ は**行列ではなくスカラー**（ただの数）で、
> 「場所 $x$ ・時刻 $t$ の温度」1つ。段階を分けると:
> | 書き方 | 固定するもの | 正体 | 大きさ |
> |---|---|---|---|
> | $u(x,t)$ | 場所も時刻も1つ | スカラー（数） | 1 |
> | $u(\cdot,t)$ | 時刻だけ、場所は全セル | ベクトル＝1枚のスナップショット | 20,696 |
> | $X$ | 全時刻ぶん並べる | 行列 | 20,696×121 |
> 上の式で $\varphi_k(x)$ は**空間の型（全セルに値を持つベクトル＝模様）**、
> $a_k(t)$ は**強さ（時刻ごとの数）**。行列 $X$ が出るのは「全点×全時刻を並べたとき」だけ。
...
> $u(x,t)=\bar u(x)+a_1(t)\,\varphi_1(x)+a_2(t)\,\varphi_2(x)+\cdots$
> （$\varphi_k$ ＝固定の型、 $a_k(t)$ ＝時間で変わる強さ）。
...
コツは、計算を**「1回だけの準備（STEP1）」と「毎回の本番（STEP2）」の2つに分ける**こと。

![実用計算フロー](img/practical_workflow.png)

### STEP 1｜準備 ― 装置を動かす“前”に、計算機で1回だけ

ここで作る3つは**別々のデータから独立に作る**（W→PODのような順番・依存はない）。
それぞれ形状・材料由来なので、あとで条件が変わっても**作り直さない**（資産）:

| 準備するもの | 中身 | どのデータから作るか |
|---|---|---|
| **W = K⁻¹H** | 温度→変形の感度行列 | **構造FEM**（剛性K・熱ひずみH）。熱源と無関係 |
| **ROMの係数 C, k, h** | 熱容量・熱抵抗・放熱 | **熱解析への校正**。材料/形状の物性 |
| **PODモード** | 温度場の型（何自由度か） | **熱スナップショット**（実際の温度場） |

作った3つを使って最後に**観測点を決める**（温度＝POD最適点、変位＝W行感度）。

> **W と POD は別物・Wから POD は出せない**（よくある誤解）。
> W は「温度→変形」の**構造**の話（熱源がどこにあるかを知らない）。
> POD は「実際にどんな温度場が生じるか」の**熱**の話（温度スナップショットが必要）。
> 別の物理・別のデータから作るので、片方からもう片方は導けない。
> 図の3箱を横に並べたのは"順番"ではなく"独立な3つの準備"の意味。

### STEP 2｜本番 ― 装置を動かし“ながら”、一定間隔で繰り返す

運転中は、下の4ステップを**一定間隔でぐるぐる回すだけ**（1周＝数秒）。
温度を数点測るだけで、全体の温度分布・熱変位・発熱量を毎サイクル推定できる:

1. **測る**：温度2点＋変位を実機センサで取得
2. **ROMで予測**：STEP1の校正済みROMを数秒回す（その時点の入力Qを与える）
3. **データ同化**：測定でROM予測を補正し、Qと全体温度を推定（STEP1で決めた観測演算子を使う）
4. **使う**：全体分布・熱変位を出力（必要ならFrontISTRやPODモードで復元）→ 機械の補正に反映

→ ④まで済んだら次のタイミングで①へ戻る。これを繰り返すのが「リアルタイム推定」。

### 条件が変わったら、どこを触るのか

- **ヒータ位置・出力・運転が変わっただけ**（＝STEP1で想定した運転範囲の中）:
  → **STEP2の入力が変わるだけ**。STEP1は一切作り直さない。**やり直しゼロ**。
- **範囲を大きく外れた新条件**（例：ヒータを全く別の場所に移設）:
  → STEP1のうち**PODモードだけ**、その1ケースを足して**再SVD（数秒）**で更新。
  それでも **W・ROM係数は不変**（構造が変わらない限り）。OpenFOAMのフル再実行は不要。

> **要点**：重い物理（W・ROM係数・PODモード）は構造由来の“資産”で、一度作れば使い回せる。
> 条件が変わって走るのは軽いSTEP2のループだけ。だから
> 「装置を作る前に準備を済ませ、運転中はリアルタイムで回す」が成立する。
