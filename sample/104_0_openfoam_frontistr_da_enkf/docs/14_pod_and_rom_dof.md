...
ここで作るものは**形状・材料・固定方法だけ**で決まるので、
あとでヒータ位置や運転条件が変わっても**作り直さない**。結果は保存して使い回す（資産）:

| 準備するもの | 中身 | なぜ条件によらないか |
|---|---|---|
| **W = K⁻¹H** | 感度行列 | 剛性K・熱ひずみHは構造の性質。熱源の位置・強さと無関係 |
| **ROMの係数 C, k, h** | 熱容量・熱抵抗・放熱 | 材料と形状の物性。入力Qの場所が変わっても回路自体は同じ |
| **PODモード** | 場の型 | 運転範囲を張る数ケースから作れば範囲内は使い回せる |
| **観測点の選定** | どこを測るか | POD最適点で決めれば固定（温度）。変位計はW行感度（105） |
