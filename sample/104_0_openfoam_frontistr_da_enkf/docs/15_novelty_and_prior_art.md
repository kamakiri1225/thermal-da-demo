...
さらに [Tan et al. 2025](https://doi.org/10.1016/j.icheatmasstransfer.2025.109869) は LM 法ながら、熱流束だけでなく熱機械荷重・物性を同時識別しています。この論文を引用せずに「従来は単一未知量のみ」と主張すると、かなり高い確率で査読者から反証されます。
...
ここから分かる重要な点は、「新規要素」が存在するというより、**既存の四つの研究流を初めて交差させるところ**に論文価値があることです。

$$
\boxed{
\begin{array}{c}
\text{thermoelastic inverse from displacement}\\
+\;\text{EnKF/OpenFOAM data assimilation}\\
+\;\text{machine-tool ROM/digital twin}\\
+\;\text{joint thermal parameter estimation}
\end{array}}
$$
...
$$
T_{\rm obs}\quad+\quad u_{\rm obs}
$$

を一つの innovation vector にして、

$$
[T,\;Q,\;h]
$$

の cross-covariance を EnKF で利用する部分が、Ansari型随伴逆解析にも Lang型工作機械KFにも Zhang型 $T+h$ EnKFにもない差分です。
...
良いか」は当然の論点であり、限界の章で必ず問われる。結論を先に:
