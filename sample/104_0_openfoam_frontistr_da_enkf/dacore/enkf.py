...
def enkf_update(Zf, y, H, R, rng, inflation=1.0, Yf=None):
    """確率的 EnKF の解析ステップ.

    引数:
        Zf        : (n_ens, n_aug) 予報アンサンブル
        y         : (n_obs,) 観測ベクトル
        H         : (n_obs, n_aug) 観測演算子(線形観測時)。Yf 指定時は未使用(None可)
        R         : (n_obs, n_obs) 観測誤差共分散
        inflation : 乗算的共分散インフレーション係数(>=1)
        Yf        : (n_ens, n_obs) 予報観測。非線形観測演算子(例: 各メンバーの
                    温度場を FrontISTR に通した変位)をメンバーごとに評価した値を
                    渡すと、H 行列なしで同じ標本共分散更新ができる。
    戻り値:
