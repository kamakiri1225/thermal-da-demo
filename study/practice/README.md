...
| グリッド | 30×30 = 900 セル |
| 状態変数 | T（温度）— q_derived = -α·L·x を表示 |
| センサ | m=[1,5,15,28] ユーザーが手動指定（ミッフィー輪郭＋背景） |
| 初期分布 | x_ss + 空間相関ノイズ（SIGMA_B=30°C） |
| パラメータ | N=50、σ_r=3°C、σ_r_eff=23.2°C（= σ_r×√60）、R_loc=8、N_ITER=60 |
...
| m センサ数 | 初期 RMSE | 最終 RMSE | 判定 |
|-----------|----------|----------|------|
| 1 | 6.1°C | 6.1°C | センサ 1 点では更新なし（当然）|
| 5 | 6.1°C | 4.9°C | わずかに改善 |
| 15 | 6.1°C | 4.2°C | 改善 |
| 28 | 6.1°C | **1.5°C** | ミッフィーがはっきり再現 |

**良かった点**
- 全 N_ITER 期間 RMSE が発散しない（インフレーション不要）
- センサを増やすほどきれいに改善（単調収束）
- q_derived（bwr カラーマップ）がミッフィー輪郭を再現
- センサ位置をユーザーが指定できる（現実的な設定）

**課題**
- 初期アンサンブルが x_ss 中心 → RMSE_0 が既に低い（真値を事前に知っている）
  - より公平なベンチマーク：x=0 または一様背景から開始
- m=1 ではほぼ更新不可（センサ 1 点の情報量が少なすぎる）

**改善策**
1. センサを 28→50 個に増やし、ミッフィーの耳・顔の細部もカバー
2. N=100 以上 → アンサンブル共分散の精度向上
3. 初期アンサンブルを背景温度（T=0 or T=50°C 均一）から開始 → より実用的な設定
4. 状態変数を Q_full（全格子の熱源）に変更し L⁻¹ 順問題を組み込む → 完全逆問題へ
5. R_loc を 6〜10 cells で調整 → ミッフィー内部の細部精度に影響
...
| `A02_miffy_esmda_SCfixTimefix_Q` | ESMDA | 手動/固定 | T→Q | **◎安定** | **1.5°C(m=28)** | 静的逆問題に最適 |
